/* 
	...
	primiterate.cl - Bryan Little 4/2025, montgomery arithmetic by Yves Gallot
	
	iterate from start# to last# mod P
...
__kernel void primiterate(	__global ulong8 * g_prime,
				__global uint * g_primecount,
				__global factor * g_factor,
				const uint start, const uint end, __global uint * g_smallprimes ){
	...
	for(uint j=start; j<end; ++j){
		...
		if(prime.s6 == prime.s3 || prime.s6 == prime.s5){
			...
			g_factor[i].nc = (prime.s6 == prime.s3) ? -((int)p) : (int)p;	// sign bit of nc is factor +1 or -1
		}
	}
